...
function parseKIFString(kifString) {
  ...
  var keyPartVal = new BigInteger(config.key.part.private_key);
  assert(keyVariant.and(new BigInteger(1)).eq(keyPartVal), 'Private key error: can not parse the kif string');
  // detect network
...
function buildPrivateKey(keyPair, network, type) {
  ...
  keyPartVal = new BigInteger(config.key.part.private_key);
