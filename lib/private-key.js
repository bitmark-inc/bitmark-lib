...
function parseKIFString(kifString) {
  ...
  assert(keyVariant.and(keyPartVal).equals(keyPartVal), 'Private key error: can not parse the kif string');
  // detect network
...
function buildPrivateKey(keyPair, network, type) {
  ...
  if (Buffer.isBuffer(keyPair)) { // keyPair is private key/seed only
    if (keyPair.length === type.prikey_length) {
      ...
      seed = keyTypeHanlder.getSeedFromPriKey(keyPair);
  ...
  keyVariantVal = keyTypeVal.shiftLeft(3).or(keyTypeVal);
...
PrivateKey.fromBuffer = function(data, network, type) {
  ...
  assert(Buffer.isBuffer(data), new TypeError('Private key error: can not recognize input format'));
...
PrivateKey.prototype.getKeyType = function(){ return this._type; };
