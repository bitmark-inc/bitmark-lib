...
function parseAddressString(addressString) {
  ...
  assert(keyVariant.and(new BigInteger(1)).eq(keyPartVal), 'Address error: this is not an address');
  // detect network
  ...
  // check checksum
  var checksum = common.sha3_256(addressBuffer.slice(0, keyVariantBuffer.length + keyType.pubkey_length));
...
function buildAddress(pubKey, network, keyType) {
  ...
  keyVariantVal.ior(new BigInteger(config.key.part.public_key.toString())); // first bit indicates address/private key
  keyVariantVal.ior(new BigInteger(network.address_value).ishln(1)); // second bit indicates net
