...
function parseAddressString(addressString) {
  ...
  // detect network
  var networkVal = keyVariant.shiftRight(1).and(new BigInt((0x03).toString())).intValue();
  var network = null;
  for (var networkName in networks) {
    if (networks[networkName].address_value === networkVal) {
      network = networks[networkName];
      break;
    }
  }
  assert(network, 'Address error: unknow network');
  // key type
