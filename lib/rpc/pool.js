...
var Pool = function(records, network){
  ...
  this.emitDBChanged = function() {
    this.emit('database-changed', this.nodes);
  };
...
Pool.prototype.cleanDatabase = function() {
  ...
  if (this.database.length !== replace.length) {
    this.emitDBChanged();
  }
...
Pool.prototype.connectToNode = function(record, callback) {
  ...
  if (!node) {
    ...
    var onConnected = function() {
      ...
      node.once('ended', onEnded);
      callback(null, node, fromCache);
    };

    node.once('connected', onConnected);
    node.once('failed', onFailed);
  } else {
...
Pool.prototype.dequeue = function() {
  ...
  var drain = queue.drain = function() {
    self.work.queue.shift();
    self.dequeue();
    if (success >= rpcConfig.broadcast.minimum) {
      work.callback(null, work.results);
    } else {
      var errorMessage = errors.length ? commonUtil.getMostAppearedValue(errors, 'message') : 'can not connect to enough nodes';
      errorMessage = 'RPC ' + work.data.method + ' failed with message: ' + errorMessage;
      work.callback(new Error(errorMessage));
    }
  };
...
var txsToRPCParams = function(txs) {
  var results = [];
  txs.forEach(function(tx) {
    results.push(tx.getRPCParam());
  });
  return results;
};


// var updateItems = function(items, key, rpcResults) {
//   items.forEach(function(item, index) {
//     var resultsForItem = collectResultForAnItemFromRPCResponse(rpcResults, key, index);
//     item.updateInfoFromRPCResponse(resultsForItem);
//   });
// };

Pool.prototype.createBitmarks = function(assets, issues, callback) {
  var param = {
    assets: txsToRPCParams(assets),
    issues: txsToRPCParams(issues)
  };
  this.pushWork(createWork('createBitmarks', param, function(error, results) {
    if (!error) {
      // updateItems(assets, 'assets', results);
      // updateItems(issues, 'issues', results);
...
// var updateTransfer = function(transfer, rpcResults) {
//   transfer.updateInfoFromRPCResponse(rpcResults);
// };
Pool.prototype.transferBitmark = function(transfer, callback){
  this.pushWork(createWork('transferBitmark', transfer.getRPCParam(), function(error, results) {
    if (error) {
      callback(error);
    } else {
      // updateTransfer(transfer, results);
      callback(null, results, {
        payId: commonUtil.getMostAppearedValue(results, 'payId'),
